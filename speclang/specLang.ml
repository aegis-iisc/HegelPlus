...
module rec Algebraic : sig
    ...
    val getTyVarsForConstructor : constructor -> Tyvar.t list 
    ...
    val getTyVars : t -> Tyvar.t list 
     ...
     let toString at = "AT"                
     let getTyVarsForConstructor c = 
      let Const {args;_} = c in 
      List.fold_left (fun acc ai -> acc@(TyD.getTyVars ai)) [] args     
     
      let getTyVars t = 
        let TD {constructors;_} = t in 
        List.fold_left (fun acc ci -> 
                            acc@(getTyVarsForConstructor ci)
                            ) [] constructors         
...
(*check if two types are same*)
 ...
 let rec same_shape t1 t2 = 
  match (t1, t2) with
    | ((Ty_alpha tvar1), (Ty_alpha tvar2)) -> true  
    | (Ty_unit, Ty_unit) -> true
    | ((Ty_ref  ta1),( Ty_ref ta2))  -> same_shape ta1 ta2 
    | ((Ty_list  ta1), (Ty_list ta2)) -> same_shape ta1 ta2
    | (Ty_int, Ty_int) -> true
    | (Ty_bool, Ty_bool)  -> true
    | (Ty_char, Ty_char) -> true        
    | (Ty_heap, Ty_heap)  -> true
    ...
    | (Ty_float, Ty_float) -> true
    | ((Ty_arrow  (t11 , t12) ), 
            Ty_arrow (t21, t22)) -> (same_shape t11 t21) && (same_shape t12 t22)
    | ( Ty_tuple (tl1), Ty_tuple (tl2) ) -> List.fold_left2 (fun acc t1i t2i -> acc && (same_shape t1i t2i)) true tl1 tl2    
    | (Ty_unknown, Ty_unknown) -> true
    | (_, Ty_unknown) ->  false
    | (Ty_unknown, _) -> false
    (* | (Ty_alg at1, Ty_alg at2) -> Algebraic.sametype at1 at2 *)
    | (Ty_star, Ty_star) -> true
    | (_, _) -> false  
(*base types from string*)    
 ...
 let rec toString t = 
      match t with
        ...
        | Ty_alpha tvar -> ("Type variable "^(Tyvar.toString tvar))
        | Ty_unit -> "Ty_unit"
 ...
 let instantiateTyvars substs tyd = 
  ...
  let rec getTyVars t = 
      match t with 
      | Ty_alpha tvar -> [tvar]
      | Ty_ref  t -> (getTyVars t) 
      | Ty_list  t -> (getTyVars t)
      | Ty_arrow  (t1 , t2)->  (getTyVars t1)@(getTyVars t2)
      | Ty_alg at -> Algebraic.getTyVars at
      | _ -> []
...
(*Qualifier schema or 
    Q = int :-> bool | \a: T,b : T. a :-> b
    qualifier len : <a : type>. [a] :-> int;
    qualifier intlistlen = len <int>
    Based on the tparameter, the relMap (in the vcencode, will map relationIDs to schemes)
    *)  
module Qualifier = struct
     type t = Qual of {
                       (* tparameter : Tyvar.t list;  *)
                       name : Var.t; (*Name of the Qualifier, len*)
                       shape : TyD.t list
                      }
    ...
    let compare_qualifier q1 q2 = 
                  
          raise (Error "Unimplemented")        
          
    (*check the sub-sort relation super_qi = R : a :-> b :-> int
            subs = [(a,int), (b, bool)]
            sub_qi prediccat  : int :-> boo :-> int
            subs |- sub_qi <: super_qi
            *)      
    let rec check_sort (subst : (TyD.t * Tyvar.t) list) sub_qi super_qi : bool = 
      (*TODO returning the*)  
      true   
...
struct
 ...
 let rec apply_relid_subst (t:expr) subst = 
    match t with 
      | V e -> t  
      | T elevc -> t  
      | X (e1, e2) -> X ((apply_relid_subst e1 subst), (apply_relid_subst e2 subst))
      | U (e1, e2) -> U ((apply_relid_subst e1 subst), (apply_relid_subst e2 subst))
      | D (e1, e2) -> D ((apply_relid_subst e1 subst), (apply_relid_subst e2 subst))
      | ADD (e1, e2) ->   ADD ((apply_relid_subst e1 subst), (apply_relid_subst e2 subst))
      | SUBS (e1, e2) ->  SUBS ((apply_relid_subst e1 subst), (apply_relid_subst e2 subst))
      
      | R (ie, arg) ->  (*actual substitition*)
            let 
            RInst {targs;sargs;args;rel} = ie in 
            let substition_for_rel = List.assoc_opt rel subst in     
            (match substition_for_rel with 
              | Some q -> let Qualifier.Qual  {name;shape} = q in 
                let ie = RInst {targs;sargs;args;rel=name} in 
                  R (ie, arg)
              | None -> raise (Error ("No substition found for the parameter "^(RelId.toString rel)))    
            )          
         
      | MultiR (ie, argl) -> (*actual substitution*)
        let RInst {targs;sargs;args;rel} = ie in 
         let substition_for_rel = List.assoc_opt rel subst in     
          (match substition_for_rel with 
            | Some q -> let Qualifier.Qual  {name;shape} = q in 
              let ie = RInst {targs;sargs;args;rel=name} in 
...
module Predicate = struct
  ...
  module RelPredicate = struct
   ...
   (*substitute Rel param *)   
  let rec apply_relid_subst t subst = 
    match t with
      | NEq (re1, re2) -> NEq (RelLang.apply_relid_subst re1 subst, 
                               RelLang.apply_relid_subst re2 subst)    
      | Eq (re1, re2) -> Eq (RelLang.apply_relid_subst re1 subst, 
                            RelLang.apply_relid_subst re2 subst)    
      | Sub (re1, re2) -> Sub (RelLang.apply_relid_subst re1 subst, 
      RelLang.apply_relid_subst re2 subst)    
      | SubEq (re1, re2) -> SubEq (RelLang.apply_relid_subst re1 subst, 
      RelLang.apply_relid_subst re2 subst)    
      | Grt (re1, re2) ->  Grt (RelLang.apply_relid_subst re1 subst, 
      RelLang.apply_relid_subst re2 subst)    
      | Q re -> Q (RelLang.apply_relid_subst re subst)
...
let rec apply_relid_subst t subst = 
  match t  with 
    | Rel rp -> Rel (RelPredicate.apply_relid_subst rp subst)
    | Exists (binds,t) ->
              Exists (binds, apply_relid_subst t subst)
    | Not t -> Not (apply_relid_subst t subst)
    | Conj (t1,t2) -> Conj ((apply_relid_subst t1 subst),
                            (apply_relid_subst t2 subst))
    | Disj (t1,t2) ->  Disj ((apply_relid_subst t1 subst),
                          (apply_relid_subst t2 subst))
    | If (t1,t2) ->  If ((apply_relid_subst t1 subst),
                          (apply_relid_subst t2 subst))
    | Iff (t1,t2) ->   Iff ((apply_relid_subst t1 subst),
                      (apply_relid_subst t2 subst))
    | Dot (t1,t2) ->   Dot ((apply_relid_subst t1 subst),
                        (apply_relid_subst t2 subst))
    | Forall (binds,t) ->
                      Forall (binds, apply_relid_subst t subst)
    | _ -> t                  
...
module RefinementType = struct 
  ...
  type t = 
    ...
    | PRT of {tyvars:Tyvar.t list;
              params : Qualifier.t list;
              refty : t}
    | Star           
    
    
    let rec getTyVars t : (Tyvar.t list) = 
      match t with 
        | Base (_,td,_) -> 
              TyD.getTyVars td
        | MArrow (e1, _,(_,td),_) -> getTyVars td 
        | Arrow ((arg, argty), body) ->  (getTyVars argty)@(getTyVars body) 
        | PRT {tyvars;params;refty} -> tyvars  
        | _ -> raise (RefTyEx ("Unimplement Case for getTyVars"))
   ...
   let rec toString rty = match rty with 
        ...
        | PRT {tyvars;params;refty} -> 
          let tyvarsString = (List.fold_left (fun acc vi -> (acc^", "^(Tyvar.toString vi))) "( " tyvars)^" )" in   
          let typedParamLists = (List.fold_left (fun acc pi -> (acc^", "^(Qualifier.toString pi))) "<< " params)^" >>" in 
          (tyvarsString^"."^(typedParamLists)^"."^(toString refty))
        | Star -> "Kind Star"
...
*We can drop the dependent function type or can use them for typing pure computations like Constructors*)      
  ...
  (*compares Base types*)
  let rec compare_types (t1:t)  (t2 :t) : bool = 
        ...
        | (PRT {params=p1;_}, PRT {params=p2;_}) -> 
            let PRT {tyvars=tv1;params=p1;refty=r1} = t1 in 
            let PRT {tyvars=tv2;params=p2;refty=r2} = t2 in
            if (List.length tv1 == List.length tv2 && List.length p1 == List.length p2) then 
              let qualifiers_comparable = List.for_all2 (fun qi qj -> Qualifier.compare_qualifier qi qj) tv1 tv2 in 
              if (qualifiers_comparable) then 
                  compare_types r1 r2
              else 
                false  
            else 
  ...
  let  rec mapBaseTy t f = match t with
    ...
    | Uncurried (arg_type_list, t2) -> 
        Uncurried (
                  ...
                  mapBaseTy t2 f )
     | _ -> raise (Error "Unimplemented mapBaseTy")             
 ...
 let sanitizeMArrow marrow = 
  ...
  let rec arity t : int = 
    ...
    | Arrow ((v1,t1),t2) -> 
            1 + (arity t2)
    | Uncurried (args_list,_) -> List.length args_list 
    | _ ->  raise (Error ("Unhandled Arity Calculation "^(toString t)))       
    
  
  let rec apply_parameter_subst t (subst : (RelId.t * Qualifier.t) list) : t = 
      match t with 
      | Base (v,t,p) ->
              (* The RelId in p changes while the parameter remains the same *)
              let p_new = Predicate.apply_relid_subst p subst in 
              Base (v, t, p_new)
      | Arrow ((v1,t1),t2) ->

            Arrow ((v1, apply_parameter_subst t1 subst), 
                                 apply_parameter_subst t2 subst)
      | Uncurried (arg_type_list, t2) -> 
          Uncurried (
                  (List.map (fun (vi, tyi) -> (vi, apply_parameter_subst tyi subst)) arg_type_list), 
                    apply_parameter_subst t2 subst)    
      | PRT {tyvars;params;refty} ->  
            raise (Error "This can't be correct, Parameters allowed only at the top level") 
      | _ -> raise (Error "Unimlemented paramter substition for the type ")
      
  let instantiate_parameters (t : t) 
                          (params : RelId.t list) 
                          (argtyvars : TyD.t list) 
                          (quals : Qualifier.t list) : t = 
       match t with
        | PRT {tyvars;refty;_} -> 
            let _ = assert (List.length params = List.length quals) in 
            let subst = List.combine params quals in
            apply_parameter_subst refty subst 
        | _ -> t (**)    
          


  (*Intantiating a parametric refinemnet type f : <p : a :-> b>. a -> {v : b | p (a)}
    where p is a parameter and a and b are tyvars.
    with two set of arguments: argtyvars = e.g. [int,bool]
    quals = e.g. greater_than : int :-> bool;
    check that the there is a valid instantiation for a, b and p 
    if valid, instantiate typevars and predicates.
   takes two arguments, a list of arguments for the typevars.e.g. a, b in the above example
    and a list of concrete qualifiers, to substitute for p
  *)  
  let instantiate_prt t (argtyvars: TyD.t list) 
                        (quals : Qualifier.t list) = 
      match t with 
        | PRT {tyvars;params;refty} -> 
             (* assert the wellformedness of arguments to typevars and
                qualifiers *)
             assert (List.length tyvars = List.length argtyvars && 
                    List.length quals = List.length params);   
             (* assert (List.for_all (fun ai -> TyD.wellformed ai) argtyvars); *)
             let type_subs = List.combine argtyvars tyvars in 
             assert (List.for_all2 (fun qai qi -> Qualifier.check_sort type_subs qai qi) argtyvars quals);
             let subst = List.combine argtyvars tyvars in 
             let instantiated_tyvars = mapTyD refty (TyD.instantiateTyvars type_subs) in 
             let (params_rel_id : RelId.t list) = List.map (fun qi -> 
                  let Qualifier.Qual {name;shape;_} = qi in name
                  ) params in 
              instantiate_parameters t params_rel_id argtyvars quals  

        | _ -> t (*monomorphic and non-parametric refinement type*)
  ...
  𝑙𝑒𝑛(𝑣1) = 𝑙𝑒𝑛(𝑥𝑠)
  ∧∀𝑢.𝑚𝑒𝑚(𝑢, 𝑣1) ⇒ 𝑚𝑒𝑚(𝑢, 𝑥𝑠)
  ∧∀𝑢, 𝑣 .𝑙𝑒𝑛(𝑣) = 𝑙𝑒𝑛(𝑣1) ∧ (𝑚𝑒𝑚(𝑢, 𝑣) ⇒ 𝑚𝑒𝑚(𝑢, 𝑣1) =⇒ ∃v2.𝑙𝑒𝑛(𝑣1) = 𝑙𝑒𝑛(𝑥𝑠) 
  ∧ ∀𝑢.𝑚𝑒𝑚(𝑢, 𝑣2) ⇒ 𝑚𝑒𝑚(𝑢, 𝑥𝑠)
  ∧ ∀𝑢, 𝑣 .𝑙𝑒𝑛(𝑣) = 𝑙𝑒𝑛(𝑣2) ∧ (𝑚𝑒𝑚(𝑢, 𝑣) ⇒ 𝑚𝑒𝑚(𝑢, 𝑣2) *)
  let diff_type (gamma : (Var.t * t) list) 
      = raise (SpecLangEx "Deprecated")
...
(*Qualifier is a new type equivalent to RelId * sort
map : 
 \forall a, b, p. 
  map_p : <(p : b :-> bool)>. (f : (x0 : a) -> {v : b | p (v) = true}) -> 
(x : [a]) -> {v : [b] | \(u : b). lmem (v, u) = true => p (u) = true };
   
*)
(* module ParamRefType =  struct
  type t = T of {params : Qualifier.t list; 
                 refty : RefinementType.t}


  let toString (T  {params; refty}) = 
      let typedParamLists = (List.fold_left (fun acc pi -> (acc^", "^(Qualifier.toString pi))) "<< " params)^" >>" in 
      (typedParamLists)^"."^(RefTy.toString refty)
  
  let generalize (sortedParams,refTy) = T {params=sortedParams; refty=refTy}
  let specialize = fun (T {params=params; refty=refty}) -> refty

  let mapTyD (T {sortedParams;refty}) f = T {params = 
                                              (List.map (fun qi -> Qualifier.mapTyD qi f) sortedParams)   
                                              ;refty=RefTy.mapTyD refty f}
...
(* The general parametric and polymprphic refinement 
\forall tyvars. ParamRefType 
map : <p : b :-> bool>. a -> { v : b | p (v) = true};    
type*)
...
struct
  ...
  let generalize = fun (tyvars, prefty) ->
    T {tyvars = tyvars; preftype = prefty}

  let specialize = fun (T {tyvars;prefty}) -> prefty
  let toString t = 
    "RefinementTypeScheme"
    
    
  (* No partial application of the instantiations 
    The is the polymorphic instantiation.   
  *)
  let instantiate (T{tyvars;preftype},tydlist) =
    let len = List.length in 
    let _ = assert (len tyvars = len tydlist) in 
    let tyvmap = List.combine tydlist tyvars
            
    in
    PRf.mapTyD preftype (TyD.instantiateTyvars tyvmap)
end
