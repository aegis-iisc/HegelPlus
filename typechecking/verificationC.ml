...
(* Return a VC capturing the subtyping relation *)
let rec fromTypeCheck (_gamma) _delta (subTy, supTy) : (t) =
        let env = {gamma=_gamma;preds=_delta} in 
        match (subTy, supTy) with 
        | (RefTy.Base (v1, t1, p1), RefTy.Base (v2, t2, p2)) ->
            let () = Printf.printf "%s" ("\n *************** SubTy \n "^(RefTy.toString subTy)) in 
            let () = Printf.printf "%s" ("\n *************** SuperTy \n "^(RefTy.toString supTy)) in 
        ...
        | (RefTy.Arrow ((v1, t1), r1), RefTy.Arrow ((v2, t2), r2)) -> 
            let vc_args = fromTypeCheck _gamma _delta (t2, t1) in 
            let vc_retType = fromTypeCheck _gamma _delta (r1, r2) in 
            (* Stubs for now, as we do not want to change the type for fromTypeCheck as this 
               will have ripple effect *)
            (* vcs_args @ vcs_retType   *)
            raise (Error "Unhanlded Arrow Case")
        | (_,_) -> 
        raise (Error "Unhandled Case Arrow or Tuple fromTypeCheck")

let rec compareTypes gamma rt1 rt2 : (t list * t list) = 
    match (rt1, rt2) with 
      | (RefTy.Base (v1, t1, p1), RefTy.Base (v2, t2, p2)) ->
            let () = Printf.printf "%s" ("\n *************** t1 \n "^(RefTy.toString rt1)) in 
            let () = Printf.printf "%s" ("\n *************** t2 \n "^(RefTy.toString rt2)) in 
            
            let _ = assert (TyD.sametype t1 t2) in 
            if (Predicate.isTrue p2) then 
              ([empty_vc], [empty_vc])
            else    
              let p2_applied = P.applySubst (v1,v2) p2 in 
              let gamma_lhs = extend_gamma (v1, rt1) gamma in 
              let gamma_rhs = extend_gamma (v2, rt2) gamma in 
              let p1_applied = P.applySubst (v2, v1) p1 in 
              ([VC (gamma_lhs, p1, p2_applied)], 
              [VC (gamma_rhs, p2, p1_applied)]) 
      | (RefTy.Arrow ((v1, t1), r1), RefTy.Arrow ((v2, t2), r2)) -> 
            let () = Printf.printf "%s" ("\n *************** t1 \n "^(RefTy.toString rt1)) in 
            let () = Printf.printf "%s" ("\n *************** t2 \n "^(RefTy.toString rt2)) in 
            let (empty_delta : Predicate.t list) = [] in 

            (* (vc_left, vc_right)  *)
            let (vc_left_for_argsCheck, vc_right_for_argsCheck) = compareTypes gamma t2 t1 in 
            
            (* (vc_left list, vc_right list)   *)
            let (vcs_left_body, vcs_right_body) = compareTypes gamma r1 r2 in 
            let vc_left = (vc_left_for_argsCheck @ (vcs_left_body)) in 

            let vc_right = (vc_right_for_argsCheck @ (vcs_right_body)) in
            (vc_left, vc_right) 

     | (_,_) -> raise (Error "comparing unrelated Types")         
