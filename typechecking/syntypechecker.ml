...
module SMTResult = ProgressLang.SMTResult
...
let typecheck (gamma : Gamma.t) (sigma:Sigma.t) (delta : Predicate.t)
    ...
    (qualifiers : SpecLang.Qualifier.t list)
    ...
    (match funType with 
     | RefTy.Arrow ((_,_),_) -> 
       ...
       (match (uncurried) with 
        | RefTy.Uncurried (formalsList, retTy) -> 
            (*@Prudent
             Two cases: when the appType return base type matches the goal spec
             and other when it does not.
             In both these cases, we need to make a call to SMT and learn a predicate  
            ...
            *)
             ...
             *)
              let actualsTypes = 
               List.map (fun vi -> 
                  ...
                  in 
                 (match vi_type with 
                    | RefTy.Base (nu, tb, phi_i) ->  
                      let phi_i_applied = Predicate.applySubst (vi_var, nu) phi_i in 
                     (vi_var, vi_type, phi_i_applied)
                   | _ -> raise (SynthesisException "EApp Typechecking : Argument Higher Order Unimplemented")  
                )   
              ) argsList in 
            ...
            if not (TyD.sametype (RefTy.toTyD appType) (RefTy.toTyD spec) ) then 
              (*@Prudent: for the baseline approach we fail on this term*)
              (* None Too restrictive *)
              (*Not keeping track of the Automata *)  
              (*A PartialTypeCheck function  does not do the exact subtype-check rather, 
                  does a check even when the return types do not match *)
              (* let vc = VC.fromPartialTypeCheck(t, apptType, spec) in  *)
         ...
         (* Re  slt.Res (None, Model.M model) *)
             ...
             (if (typechecks) then 
                ...
                None)
        | _ -> raise (SynthesisException "Curried types must be RefTy.Curry")  
      )        
     (* Result.Res (None, Model.M model) *)
  ...
  | Ecapp (funName, argsList) ->    
    (* @Prudent: The Constructor application will only occur when we maching return type
    *)
...
(* 
In the QTA world, this is an application of a transition
The rule should be more general
\Gamma |- (f : t) [args] => \tau 
   f : x : {v : a | phi1} -> y : {v : b}
              The HOF application rule*)
let rec synthesizeType4Transition 
    (gamma: Gamma.t) 
    (ftype : RefTy.t)
    (arg_var : Var.t) 
    (arg_ty : RefTy.t) : (RefTy.t) = 
  let _ = Printf.printf "%s" ("\n Synthesizing the Type for Complete/partial Function Call") in 
  match ftype with 
  | RefTy.Arrow ((vf,tf), body_type) ->
    let app_type =  RefTy.applySubsts [(arg_var, vf)] body_type in 
    app_type
  | RefTy.Base (_,_,_) -> ftype
  | _ -> raise (SynthesisException ("Unimpl synthesizeType for "^(RefTy.toString ftype)))



let rec preciseType4Exp gamma sigma delta (t : Syn.monExp) : (RefTy.t) = 
  (*undefined*)
  let () = Printf.printf "%s" ("\n Typechecking the Term "^(Syn.monExp_toString t)) in 
  match t with 
  | Eapp (funExp, argsList) (*Eapp foo [x1, x2, x3,...xn] *) -> 
