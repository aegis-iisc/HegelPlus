...
let rec getTerminalCall (t : monExp) : Var.t = 
    match t with 
        | Eapp (fname, lais) -> 
            (match fname with 
                | Evar v -> v 
                | _ -> raise (IncorrectExp "Function Name must be a Variable")
            )
        | Eret me  -> getTerminalCall me
        | Elet (x, me1, me2) -> getTerminalCall me2 
        | _ -> raise (IncorrectExp "Unhanlded Case of TerminalCall")

(*return the penultimate?? term for a term*)
let rec getPrevious (t : monExp) : (monExp option) = 
    match t with 
        | Eapp (fname, lais) -> 
            (*return the last element in the calls lile f (e1,..., en)*)
            if (List.length lais > 0) then 
                Some (List.hd (List.rev (lais)))
            else
                None 
        | Eret me  -> 
                getPrevious me
        | Elet (x, me1, me2) -> getPrevious me2 
        | Evar _ -> None
        | _ -> raise (IncorrectExp "Unhanlded Case of TerminalCall")
...
(* Since our environment now is a map from 
   monExp :-> RefTy, we need to define the notion of equality for monExp
   *)
