...
module Printf = struct 
  ...
  let originalPrint d s =  Printf.printf d "" 
...
let discharge (VC.T (tydbinds, anteP, conseqP) as vc) 
  ...
  in
  (*This needs to change as now the relMap will be a mapping from Relation-name to RelationSortScheme*)
  let processQualifiers (tyMap, constMap, relMap) qual = 
    let () = Printf.printf "%s" (" \n processQualifiers VCE "^(SpecLang.Qualifier.toString qual)) in 
    let SpecLang.Qualifier.Qual {name;shape} = qual in 
...
let res = 
  if (String.equal outbuf "unsat\n") then 
    ...
    Solver.SATISFIABLE (* the ~p has a satisfiable solution so P is not valid and the typechecking fails, 
                          we also get a model which we use to get a progress predicate 
                         *)
...
(* 
  ...
  let model = Solver.get_model solverDischarged in 
...
let discharge_VCS (vcs : VC.standardt list)  vars qualifiers : (bool list) = 
  (*discharge VCs*)
  let  dischargeVC i vc =
          let () = Printf.printf "%s" ("\n ***************** VC number "^(string_of_int i)^"\n \n "^(VC.string_for_vc_stt vc)^"***************************") in 
          match (discharge vc vars qualifiers) with   
          | Success -> true
          | Undef -> false
          | Failure -> false
  in         
  let list_results = List.mapi (fun i vci -> 
                    dischargeVC i vci) vcs in   
  list_results
