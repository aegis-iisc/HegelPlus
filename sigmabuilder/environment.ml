...
module TypingEnv = 
  struct
    ...
    (* diff = Bigger/Smaller *)
    let diff (bigger : t) (smaller : t) = 
      if (List.length bigger <= List.length smaller) then 
        smaller
      else
        List.filter 
                (fun (tk, tv) -> 
                  let in_smaller = 
                    (try 
                      Some (find smaller tk) 
                    with 
                      | _ -> None
                    )    
                 in 
                 match in_smaller with 
                    | Some _ -> true
                    | None -> false
                ) bigger
    ...
    let lambdas4RetType t (rt:TypingEnvValue.t)  : ((TypingEnvKey.t*TypingEnvValue.t) list) =
      ...
      in 
     loop t []
    (*return all functions in the environment*)
    let allLambdas t : ((TypingEnvKey.t*TypingEnvValue.t) list) =
        let rec loop gamma (lambdas : ((TypingEnvKey.t*TypingEnvValue.t) list) ) = 
            (match gamma with 
              | [] -> lambdas 
              | (vi, rti) :: xs -> 
                  (match rti with 
                  | RefinementType.Arrow ((_,_),_) ->    
                    let lambdas = lambdas@[(vi, rti)] in 
                    loop xs lambdas
                  | _ -> loop xs lambdas)
            ) 
         in
...
(*A map from component name to a monExp*)
